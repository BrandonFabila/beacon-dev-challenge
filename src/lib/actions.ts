...
export async function updateProduct(
  ...
  try {
    ...
    const updated = await Product.findOneAndUpdate(
      { slug },
      { name, slug: newSlug, description, price, category, brand, stock, image, requiresPrescription },
      { new: true },
    );
    if (!updated) {
    ...
    if (message.includes("duplicate key") || message.includes("E11000")) {
      return {
        error: "Ya existe un producto con un nombre similar. Elige un nombre diferente.",
      };
    }
